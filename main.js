...
function dodajNaziv() {
    ...
    if (!pritisnjeno) {
        pritisnjeno = false;
        naziv.innerHTML = `<span class="fw-bold">${textNaziv}</span><button style="border: 0; background: transparent;" onclick="izbrisiSebe(dodanNaziv)"><img src="https://img.icons8.com/ios-glyphs/30/000000/xbox-x.png"/></button>`;
        dodanNaziv.appendChild(naziv);
        document.getElementById("dodanNaziv").value = "";
        reset();
    } else {
    ...
    } let gumbTiska = document.getElementById("printpagebutton");
}
